## 功能概述
图片标签功能实现图片的标签识别，返回图片中置信度较高的主题标签。图片标签识别请求包属于 GET 请求，请求时需要携带签名。
...
```shell
GET /?ci-process=detect-label&detect-url=<detect-url> HTTP/1.1
...
```shell
GET /?ci-process=detect-label&detect-url=xxx HTTP/1.1
