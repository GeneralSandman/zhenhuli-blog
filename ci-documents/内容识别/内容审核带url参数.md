## 功能概述
内容审核功能实现图片的额涉黄、涉政、涉暴恐以及广告引导类图片、视频的扫描。内容审核请求包属于 GET 请求。
...
```shell
GET /?ci-process=sensitive-content-recognition&detect-type=<type>&detect-url=<detect-url> HTTP/1.1
...
```shell
GET /?ci-process=sensitive-content-recognition&detect-type=porn,politics&detect-url=xxxxxxxx HTTP/1.1
